...
class Inversion(DiffusionPipeline):
    ...
    def backward_diffusion_ptp(self,
        ...
        for i, t in enumerate(tqdm(model.scheduler.timesteps[-start_time:])):
            ...
            trans = (transfer_bool[t // 100] == '*')
            if uncond_embeddings_ is None:
                if trans:
                    context = torch.cat([uncond_embeddings[i].expand(*text_embeddings.shape), text_embeddings])
                else:
                    context = torch.cat([uncond_embeddings[i].expand(*text_embeddings.shape),
                                         text_embeddings[0].expand(*text_embeddings.shape)])
            else:
    ...
    def null_optimization_path(self, latent_list,cond_embeddings,num_inner_steps, epsilon, guidance_scale=7.5,num_ddim_steps=50):
        ...
        for i in range(num_ddim_steps):
            ...
            for j in range(num_inner_steps):
                noise_pred_uncond = self.get_noise_pred_single(latent_cur, t, uncond_embeddings)
                noise_pred = noise_pred_uncond + guidance_scale[int(t/100)]  * (noise_pred_cond - noise_pred_uncond)
                latents_prev_rec = self.prev_step(noise_pred, t, latent_cur)
                loss = nnf.mse_loss(latents_prev_rec, latent_prev)
                optimizer.zero_grad()
                loss.backward()
